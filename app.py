...
def fetch_regrid_parcel(loc: dict, token: str) -> dict | None:
    ...
    # Build context path for Regrid: /us/fl/okaloosa/niceville
    ...
    # --- Attempt 1: v2 address endpoint (best for address lookup) ---
    if street:
        resp = safe_get(
            "https://app.regrid.com/api/v2/parcels/address",
            params={"query": street, "path": path, "limit": 1, "token": token},
        )
        if resp:
            try:
                props = _regrid_extract_properties(resp.json())
                if props:
                    return props[0]
            except Exception:
                pass

        # Try without city in path (sometimes county is enough)
        if city_slug:
            path_no_city = f"/us/{state_code}/{county_slug}" if county_slug else f"/us/{state_code}"
            resp = safe_get(
                "https://app.regrid.com/api/v2/parcels/address",
                params={"query": street, "path": path_no_city, "limit": 1, "token": token},
            )
            if resp:
                try:
                    props = _regrid_extract_properties(resp.json())
    ...
    query = street
    if not query:
        query = loc.get("raw_input", "")
    if query:
        resp = safe_get(
            "https://app.regrid.com/api/v1/search.json",
            params={"query": query, "context": path, "limit": 1, "token": token},
        )
        if resp:
            try:
                props = _regrid_extract_properties(resp.json())
    ...
    # --- Attempt 3: lat/lon with wider radius ---
    lat = loc.get("lat")
    lon = loc.get("lon")
    if lat and lon:
        resp = safe_get(
            "https://app.regrid.com/api/v1/search.json",
            params={"lat": lat, "lon": lon, "radius": 100, "limit": 3, "token": token},
        )
        if resp:
            try:
                props = _regrid_extract_properties(resp.json())
