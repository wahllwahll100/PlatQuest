...
def _census_match_to_loc(match: dict, raw_input: str) -> dict:
    ...
    # Extract the actual house number from the user's original input,
    # NOT from fromAddress (which is the range start, e.g. 199 instead of 119).
    # Parse the leading number from the original input text.
    house_number = ""
    num_match = re.match(r'^\s*(\d+)', raw_input)
    if num_match:
        house_number = num_match.group(1)
    else:
        # Fallback: try to parse from the matched address
        num_match2 = re.match(r'^\s*(\d+)', matched_addr)
        if num_match2:
            house_number = num_match2.group(1)
...
def _regrid_extract_properties(data: dict, key: str = "results") -> list[dict]:
    """Extract properties from Regrid v1 or v2 response."""
    # v2 returns {"parcels": {"type": "FeatureCollection", "features": [...]}}
    parcels_fc = data.get("parcels")
    if parcels_fc and isinstance(parcels_fc, dict):
        features = parcels_fc.get("features", [])
        return [f.get("properties", {}) for f in features if f.get("properties")]

    # v1 returns {"results": [{"properties": {...}}, ...]}
    results = data.get(key, [])
    if results:
        return [r.get("properties", {}) for r in results if r.get("properties")]

    return []


def fetch_regrid_parcel(loc: dict, token: str) -> dict | None:
    """
    Look up the parcel for a specific address using Regrid.
    Strategy: v2 address search → v1 address search → v1 lat/lon fallback.
    Stores debug info in st.session_state["_regrid_debug"].
    """
    debug = []
    street = loc.get("street_address", "")
    state_code = loc.get("state_code", "").lower()
    county_slug = loc.get("county", "").replace(" County", "").replace(
        " Parish", "").strip().lower().replace(" ", "-")
    city_slug = loc.get("city", "").strip().lower().replace(" ", "-")

    path = f"/us/{state_code}"
    if county_slug:
        path += f"/{county_slug}"
        if city_slug:
            path += f"/{city_slug}"

    def _try_regrid(url, params, label):
        time.sleep(SCRAPE_DELAY)
        try:
            r = requests.get(url, params=params, timeout=REQUEST_TIMEOUT)
            debug.append(f"**{label}:** HTTP {r.status_code}")
            if r.status_code == 200:
                data = r.json()
                props = _regrid_extract_properties(data)
                debug.append(f"  → {len(props)} parcel(s) returned")
                if not props:
                    # Show keys so we can see the response structure
                    debug.append(f"  → Response keys: {list(data.keys())}")
                    # Show first 300 chars of response for debugging
                    raw_text = r.text[:300]
                    debug.append(f"  → Raw: `{raw_text}`")
                return props
            else:
                debug.append(f"  → Error body: `{r.text[:200]}`")
        except Exception as e:
            debug.append(f"**{label}:** Exception: {e}")
        return []

    # --- Attempt 1: v2 address endpoint ---
    if street:
        props = _try_regrid(
            "https://app.regrid.com/api/v2/parcels/address",
            {"query": street, "path": path, "limit": 1, "token": token},
            f"v2 address: query='{street}', path='{path}'",
        )
        if props:
            st.session_state["_regrid_debug"] = debug
            return props[0]

        # Try without city
        if city_slug:
            path_no_city = f"/us/{state_code}/{county_slug}" if county_slug else f"/us/{state_code}"
            props = _try_regrid(
                "https://app.regrid.com/api/v2/parcels/address",
                {"query": street, "path": path_no_city, "limit": 1, "token": token},
                f"v2 address (no city): query='{street}', path='{path_no_city}'",
            )
            if props:
                st.session_state["_regrid_debug"] = debug
                return props[0]

    # --- Attempt 2: v1 address query ---
    query = street or loc.get("raw_input", "")
    if query:
        props = _try_regrid(
            "https://app.regrid.com/api/v1/search.json",
            {"query": query, "context": path, "limit": 1, "token": token},
            f"v1 query: query='{query}', context='{path}'",
        )
        if props:
            st.session_state["_regrid_debug"] = debug
            return props[0]
    ...
    if lat and lon:
        props = _try_regrid(
            "https://app.regrid.com/api/v1/search.json",
            {"lat": lat, "lon": lon, "radius": 200, "limit": 5, "token": token},
            f"v1 lat/lon: ({lat}, {lon}), radius=200m",
        )
        if props:
            st.session_state["_regrid_debug"] = debug
            return props[0]
...
def fetch_regrid_nearby(loc: dict, token: str, radius: int = 500,
                        limit: int = 50) -> list[dict]:
    """Fetch nearby parcels for neighbourhood context."""
    lat = loc.get("lat")
    lon = loc.get("lon")
    ...
    if resp:
        try:
            return _regrid_extract_properties(resp.json())
        except Exception:
            pass
    return []
...
def main():
    ...
    if regrid_key:
        with st.spinner("Querying Regrid for parcel & subdivision data…"):
            parcel = fetch_regrid_parcel(loc, regrid_key)
            nearby = fetch_regrid_nearby(loc, regrid_key)

        # Debug info (collapsible)
        with st.expander("🔧 Regrid API Debug Info", expanded=False):
            st.write(f"**Street address sent:** `{loc.get('street_address', '')}`")
            st.write(f"**Lat/Lon:** `{loc.get('lat')}, {loc.get('lon')}`")
            county_slug = loc.get("county", "").replace(" County", "").replace(
                " Parish", "").strip().lower().replace(" ", "-")
            city_slug = loc.get("city", "").strip().lower().replace(" ", "-")
            sc = loc.get("state_code", "").lower()
            path = f"/us/{sc}"
            if county_slug:
                path += f"/{county_slug}"
                if city_slug:
                    path += f"/{city_slug}"
            st.write(f"**Regrid path:** `{path}`")
            st.write(f"**Parcel found:** {'Yes' if parcel else 'No'}")
            st.write(f"**Nearby parcels:** {len(nearby)}")
            st.write("---")
            st.write("**API call log:**")
            for line in st.session_state.get("_regrid_debug", []):
                st.markdown(line)
            if parcel:
                st.write("---")
