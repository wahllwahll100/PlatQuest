...
FDOT_BASE = "https://gis.fdot.gov/arcgis/rest/services/Parcels/FeatureServer"
FL_COUNTY_LAYERS = {
...
def fetch_fdot_parcel(lat, lon, county_name):
    """
    Query FDOT's ArcGIS FeatureServer for the parcel at this point.
    Tries the expected layer ID plus +/-1 to handle numbering uncertainty.
    Returns (attributes_dict, status_string).
    Stores debug in session state.
    """
    debug = []
    base_id = _get_fl_layer_id(county_name)
    if base_id is None:
        msg = f"County '{county_name}' not found in FL layer map"
        debug.append(msg)
    ...
    # Try the expected layer, then +1, then -1
    for offset in [0, 1, -1]:
        layer_id = base_id + offset
        url = f"{FDOT_BASE}/{layer_id}/query"
        debug.append(f"**Trying layer {layer_id}** (base {base_id}, offset {offset:+d})")
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                if "error" in data:
                    debug.append(f"ArcGIS error: {data['error'].get('message', '')}")
                    continue
                ...
                debug.append(f"Response: `{r.text[:200]}`")
        except Exception as e:
            debug.append(f"Exception: {e}")

    st.session_state["_fdot_debug"] = debug
    return None, f"No parcel found after trying layers {base_id-1}, {base_id}, {base_id+1}"


def fetch_fdot_nearby(lat, lon, county_name, radius_m=500):
    """Query for parcels within a radius (meters) for neighbourhood context."""
    layer_id = _get_fl_layer_id(county_name)
    if layer_id is None:
        return []
