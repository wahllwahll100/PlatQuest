...
def fetch_fdot_parcel(lat, lon, county_name):
    ...
    Returns dict of parcel attributes or None.
    """
    layer_id = _get_fl_layer_id(county_name)
    if layer_id is None:
        return None, f"County '{county_name}' not found in FL layer map"
    ...
    if not resp:
        return None, "FDOT server did not respond"

    try:
        data = resp.json()
        features = data.get("features", [])
        if not features:
            return None, f"No parcel found at ({lat:.5f}, {lon:.5f}) in layer {layer_id}"
...
def main():
    ...
    with st.expander("🔧 Debug Info", expanded=False):
        ...
        st.write(f"**FDOT Layer ID:** `{layer_id}`")
